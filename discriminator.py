...
class DiscriminatorNet(torch.nn.Module):
    ...
    """
    def __init__(self, in_features, leakyRelu=0.2, dropout=0.3):
...
def train_discriminator(optimizer, discriminator, loss, real_data, fake_data):
    ...
    error_real = loss(prediction_real, real_data_target([real_data.size(0), real_data.size(1)]))
    error_real.backward()

    # 1.2 Train on Fake Data
    prediction_fake = discriminator(fake_data)
    # Calculate error and backpropagate
    error_fake = loss(prediction_fake, fake_data_target([real_data.size(0), real_data.size(1)]))
    error_fake.backward()
